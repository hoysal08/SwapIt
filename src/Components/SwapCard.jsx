...
function SwapCard({ swapId, NFTobj }) {
  ...
  async function fetchmetadata() {
    if (
      NFTobj?.tokenId !== undefined &&
      NFTobj?.contractaddres !== undefined &&
      correctcntaddress
    ) {
      let response = await alchemy.nft.getNftMetadata(
        NFTobj.contractaddres,
        NFTobj.tokenId
      );
  ...
  return (
    <div>
      <Box pl="2%">
        <Text pt="2%" pl="4%" as="samp" fontSize="xl">
          Offers for SwapID : {swapId}
        </Text>
        <Box pl="10%" pt="2%">
          <Box
            maxW="sm"
            ...
            <Box px="6" py="2">
              <Flex direction="row" justify="space-between">
                ...
                <Box
                  ...
                  ml="2"
                >
                  {formatcontractaddress(NFTobj?.contractaddres)} &bull; #
                  {NFTobj?.tokenId}
                </Box>
              </Flex>
              <Box
