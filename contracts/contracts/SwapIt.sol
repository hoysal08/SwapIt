...
contract SwapIt {
    ...
    function isApprovedfunc(address _tokenContract, uint256 _tokenId) public view returns(bool) {
        // add Approval
            return IERC721(_tokenContract).getApproved(_tokenId) == address(this);
        
    }

    function getCSwapTokenAddress(uint256 _swapId,uint256 _index) public view returns(address){
           return swaps[_swapId].swapTokens[_index];
    }
    function getCSwapTokenId(uint256 _swapId,uint256 _index) public view returns(uint){
           return swaps[_swapId].swapTokenIds[_index];
    }  

   function getOSwapTokenAddress(uint256 _swapId,uint256 _index) public view returns(address){
           return offers[_swapId][_index].offerTokens[_index];
    }
    function getOSwapTokenId(uint256 _swapId,uint256 _index) public view returns(uint){
           return offers[_swapId][_index].offerTokenIds[_index];
    }  
